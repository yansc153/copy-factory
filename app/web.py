...
<style>
:root{{color-scheme:light;--line:#eff3f4;--muted:#536471;--ink:#0f1419;--blue:#1d9bf0;--soft:#f7f9f9}}
...
a{{color:inherit;text-decoration:none}} a:hover{{text-decoration:underline}} main{{max-width:1180px;margin:0 auto}}
.shell{{display:grid;grid-template-columns:220px minmax(0,640px) 300px;min-height:100vh}}
.rail{{border-right:1px solid var(--line);padding:22px 18px;position:sticky;top:0;height:100vh}}
.brand{{font-size:28px;font-weight:800;margin:0 0 28px}} .nav{{display:grid;gap:10px}} .nav a,.sync-btn,.logout{{border-radius:999px;padding:12px 16px;font-weight:700}}
...
h1{{font-size:22px;margin:0}} h2{{font-size:17px;line-height:1.35;margin:0 0 4px}} .hint,.meta{{color:var(--muted);font-size:13px}}
...
.head{{display:flex;gap:6px;align-items:center;flex-wrap:wrap}} .source{{font-weight:800}} .dot{{color:var(--muted)}} .pill{{border:1px solid var(--line);border-radius:999px;padding:2px 8px;font-size:12px;color:var(--muted)}}
...
.copy{{white-space:pre-wrap;line-height:1.48;margin:10px 0 0}} .media{{display:flex;gap:8px;flex-wrap:wrap;margin-top:10px}} .media span,.media a{{border:1px solid var(--line);border-radius:999px;padding:4px 9px;color:var(--muted);font-size:12px}}
.actions{{display:flex;gap:18px;margin-top:12px;color:var(--muted);font-size:13px}} .actions a{{color:var(--blue);font-weight:700}}
.side{{padding:18px;position:sticky;top:0;height:100vh}} .card{{background:var(--soft);border-radius:8px;padding:16px;margin-bottom:14px}} .stat{{display:flex;justify-content:space-between;margin:8px 0}}
...
.detail{{display:grid;grid-template-columns:minmax(0,640px) 300px;gap:0;max-width:940px;margin:0 auto;border-left:1px solid var(--line);border-right:1px solid var(--line)}}
...
<section class="feed">
<div class="topbar"><h1>今日待审核池</h1><p class="hint">像时间线一样审文案：先看来源和图片，再点进编辑。</p></div>
...
"""
    ...
    def tweet(self, row) -> str:
        ...
        media_chips = "".join(f"<span>图片 {i}</span>" for i, _ in enumerate(media[:4], 1))
...
<p class="copy">{copy}</p>
<div class="media">{media_chips or '<span>无图片</span>'}</div>
<div class="actions"><a href="/items/{row['id']}">编辑文案</a><span>原文</span><span>图片引用 {len(media)}</span></div>
</div>
</article>"""
    ...
    def item_page(self, item_id: int) -> None:
        ...
        if not item:
            self.send_error(404)
            return
        media = "\n".join(f"<li>{html.escape(str(url))}</li>" for url in json.loads(item["media_urls"]))
        body = f"""
<div class="detail">
